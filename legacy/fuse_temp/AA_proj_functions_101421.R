...
get_js_v2 <- function(m, conf.level = NULL, B = 1000, seed = NULL) {
  # m: a numeric matrix with rows = observations, columns = parameters
  # conf.level: confidence level for intervals (e.g., 0.95)
  ...
  mu0 <- colMeans(m, na.rm = T)
  
  # Global mean
  mbar <- mean(mu0, na.rm = T)
  
  # Compute pooled variance estimate
  ss <- sum((m - matrix(rep(mu0, n), nrow = n, byrow = T))^2, na.rm = T)
  sigma2_hat <- ss / (p * (n - 1))
  
  # Compute James-Stein shrinkage factor
  denom <- sum((mu0 - mbar)^2)
  
  if (denom == 0) {
    # If all means identical, no shrinkage is possible
    js_est <- mu0
  } else {
    cval <- 1 - ((p - 3)*sigma2_hat)/denom
    cval <- max(0, cval)  # positive part
    js_est <- mbar + cval * (mu0 - mbar)
  }
  
  # If no CIs requested, just return the estimates
  if (is.null(conf.level)) {
    return(js_est)
  }
  
  # If confidence intervals are requested, use parametric bootstrap
  alpha <- 1 - conf.level
  
  # Storage for bootstrap estimates
  js_boot <- matrix(NA, nrow = B, ncol = p)
  
  for (b in 1:B) {
    # Parametric bootstrap: simulate new data from N(mu0, sigma2_hat)
    # mu0 are treated as the "true means"
    M_boot <- matrix(rnorm(n * p, mean = rep(mu0, each = n), sd = sqrt(sigma2_hat)),
                     nrow = n, ncol = p)
    
    # Recompute JS estimate on bootstrap sample
    mu0_b <- colMeans(M_boot, na.rm = T)
    mbar_b <- mean(mu0_b, na.rm = T)
    
    ss_b <- sum((M_boot - matrix(rep(mu0_b, n), nrow = n, byrow = T))^2, na.rm = T)
    sigma2_hat_b <- ss_b / (p * (n - 1))
    denom_b <- sum((mu0_b - mbar_b)^2)
    
    if (denom_b == 0) {
      js_boot[b, ] <- mu0_b
    } else {
      cval_b <- 1 - ((p - 3)*sigma2_hat_b)/denom_b
      cval_b <- max(0, cval_b)
      js_boot[b, ] <- mbar_b + cval_b * (mu0_b - mbar_b)
    }
  }
  
  # Compute empirical confidence intervals from bootstrap distribution
  lower <- apply(js_boot, 2, quantile, probs = alpha/2)
  upper <- apply(js_boot, 2, quantile, probs = 1 - alpha/2)
  
  # Return a data frame with estimates and CIs
  res <- data.frame(
    Estimate = js_est,
    Lower_CI = lower,
    Upper_CI = upper
...
pivot_norm_score_ss <-  function(df, df.funsum_all, ls.funsum_ss, include_LOF=F, score_type="norm_raw_score"){
  if (include_LOF){
    aa_list = unlist(strsplit("RHKDESTNQCGPAVILMFYW*", split = ""))
  } else {
    aa_list = unlist(strsplit("RHKDESTNQCGPAVILMFYW", split = ""))
  ...
  df.funsum_all = df.funsum_all[aa_list, aa_list]
  for (ss in names(ls.funsum_ss)){
    df.funsum = ls.funsum_ss[[ss]]
    df.funsum = df.funsum[aa_list, aa_list]
    ls.funsum_ss[[ss]] = df.funsum
  }
  ind = which((df$aaref %in% aa_list) & (df$aaalt %in% aa_list))
  df = df[ind,]
  
  id_cols = c("gene", "aapos", "aaref", "ss", "acc", "ss2")
  df.out = df %>% group_by(gene, aapos, aaref, aaalt, ss, acc, ss2) %>% 
    summarise(raw_score = mean(raw_score), norm_raw_score = mean(norm_raw_score)) %>% 
    pivot_wider(id_cols = all_of(id_cols), names_from = "aaalt", values_from = all_of(score_type))
  for (aa in aa_list){
    if (!aa %in% colnames(df.out)){
      df.out[[aa]] = NA
    }
  }
  df.out$pos_mean = NA
  df.out$score_ct = 0
  df.out = df.out %>% select(all_of(c(id_cols, "score_ct", "pos_mean", aa_list)))
  temp = as.matrix(df.out[, aa_list])
  ...
  for (i in 1:nrow(df.out)){
    ...
    if (length(ind)){
      if (is.na(ss2)){
        ...
        temp2 = temp[i,ind] - ls.funsum_ss[[ss2]][aaref, aa_list[ind]]
      }
      pos_mean = mean(as.numeric(temp2))
    }
    df.out$pos_mean[i] = pos_mean
    df.out$score_ct[i] = length(ind)
  }
  
  return(df.out)
}

align_func_dssp_aapos <- function(df.func, df.dssp, query_seq_len = 10){
  df.refseq = data.frame(aapos = min(df.func$aapos):max(df.func$aapos))
  df.refseq = merge(df.refseq, unique(df.func[,c("aapos", "aaref")]), all.x = T)
  temp = df.refseq %>% drop_na()
  if (nrow(temp) < query_seq_len){
    query_seq_len = nrow(temp)
  }
  query_aapos_end = temp$aapos[query_seq_len]
  df.refseq$aaref[is.na(df.refseq$aaref)] = "."
  query_aapos_end_ind = which(df.refseq$aapos == query_aapos_end)
  query_seq = paste0(df.refseq$aaref[1:query_aapos_end_ind], collapse = "")
  
  df.dssp = df.dssp %>% mutate(aapos = respdb, aaref = aa, acc = sasa) %>% select(aapos, aaref, ss, acc)
  df.dssp$ss2 = ss_list[df.dssp$ss]
  if (min(df.dssp$aapos) == 2){
    temp = data.frame(aapos = 1, aaref = "M", ss = NA, acc = NA, ss2 = NA)
    df.dssp = rbind(temp, df.dssp)
  }
  df.refseq2 = data.frame(aapos = min(df.dssp$aapos):max(df.dssp$aapos))
  df.refseq2 = merge(df.refseq2, df.dssp[,c("aapos", "aaref")], all.x = T)
  df.refseq2$aaref[is.na(df.refseq2$aaref)] = "."
  refseq2 = paste0(df.refseq2$aaref, collapse = "")
  
  pos_shift = nchar(str_split(pattern = query_seq, refseq2)[[1]][1])
  df.func$aapos = df.func$aapos + pos_shift - min(df.func$aapos) + min(df.dssp$aapos)
  df.func_pos_adjusted = merge(df.func, df.dssp, all.x = T)
  
  return(df.func_pos_adjusted)
}
...
ssv_to_v2p <- function(ssv_path){
  df.v2p = read_delim(ssv_path, delim = " ", col_names = c("chr", "na", "pos", "ref", "alt", "AF", "pid_str")) %>%
    select(-na) %>% mutate(pid_str = str_sub(pid_str, 1, -2))
  return(df.v2p)
}
...
get_FUNSUM_v2 <- function(df, include_LOF=F, score_col="raw_score"){
  ...
  # filter out rows without amino acid changes
  df = df %>% filter(!is.na(aaref) & !is.na(aaalt) & aaref %in% aa_list & aaalt %in% aa_list)
  
  # initialize the output dataframe
  df.func_score = df %>% select(aapos, aaref) %>% distinct() %>% mutate(pos_mean = NA)
  
  # initialize the temporary matrix
  temp = matrix(NA, nrow = nrow(df.func_score), ncol = length(aa_list))
  colnames(temp) <- aa_list
  
  # fill the temporary matrix
  for (i in 1:nrow(df.func_score)){
    j = which(df$aapos == df.func_score$aapos[i])
    temp[i, df$aaalt[j]] = df[[score_col]][j]
  }
  
  # calculate the position mean
  df.func_score$pos_mean = get_js(t(temp))
  
  # subtract positional mean to get the substitution component
  temp = temp - df.func_score$pos_mean
  
  # combine the position mean and the substitution component
  df.func_score = cbind(df.func_score, temp)
  
  # initialize the FUNSUM matrix
  df.funsum = matrix(NA, nrow = length(aa_list), ncol = length(aa_list))
  rownames(df.funsum) = aa_list
  colnames(df.funsum) = aa_list
  
  # fill the FUNSUM matrix
  for (aa in aa_list){
    ind = which(df.func_score$aaref == aa)
...
de_noise_1gene <- function(df, include_LOF = T, missense_only = F, df.funsum = NULL){
  require(dplyr)
  ...
  # assign functional class if not present
  if (!"functional_class" %in% colnames(df)){
    df = get_functional_class(df)
  }
  
  # calculate the FUNSUM matrix if not provided
  if (is.null(df.funsum)){
    if (missense_only){
      df = df %>% filter(functional_class == "MIS")
    }
    
    df.funsum = get_FUNSUM_v2(df, include_LOF = include_LOF, score_col = "norm_raw_score")
  }
  
  # convert FUNSUM to tabular format
  ...
  # collapse rows with the same amino acid substitutions
  df = df %>% group_by(gene_aa_str) %>%
    summarise(gene = first(gene), aapos = first(aapos), aaref = first(aaref), aaalt = first(aaalt), 
              raw_score = mean(raw_score), norm_raw_score = mean(norm_raw_score))
  
  ## calculate positional component
  df.pos_score = df %>% group_by(gene, aapos) %>% summarise(aaref = first(aaref), pos_mean = NA)
  df.pos_score$gene_aapos = paste0(df.pos_score$gene, "---", df.pos_score$aapos)
  ...
  # pb = txtProgressBar(min = 1, max = nrow(df.pos_score), initial = 1, style = 3) 
  for (i in 1:nrow(df.pos_score)){
    j = which(df$gene == df.pos_score$gene[i] & df$aapos == df.pos_score$aapos[i])
    temp[i, df$aaalt[j]] = df$norm_raw_score[j]    
    # setTxtProgressBar(pb,i)
  }
  # close(pb)
...
get_all_snp_aa_subs <- function(cds, return_unique = TRUE) {
  stopifnot(is.character(cds), length(cds) == 1L)
  # Clean & validate
  cds <- toupper(gsub("\\s+", "", cds))
  if (!grepl("^[ACGT]+$", cds)) stop("cds must contain only A/C/G/T.")
  if (nchar(cds) %% 3 != 0) stop("cds length must be a multiple of 3.")
  
  # Split into codons
  n_codons <- nchar(cds) / 3L
  codons <- substring(cds, first = seq(1, by = 3, length.out = n_codons),
                      last  = seq(3, by = 3, length.out = n_codons))
  ref_aas <- vapply(codons, translate_codon, character(1))
  
  bases <- c("A","C","G","T")
  out_list <- vector("list", length = n_codons * 3L * 3L)  # upper bound
  k <- 0L
  
  for (i in seq_len(n_codons)) {
    ref_codon <- codons[i]
    aaref <- ref_aas[i]
    codon_chars <- strsplit(ref_codon, "", fixed = TRUE)[[1]]
    for (pos in 1:3) {
      nt_ref <- codon_chars[pos]
      for (nt_alt in bases[bases != nt_ref]) {
        alt_codon_chars <- codon_chars
        alt_codon_chars[pos] <- nt_alt
        alt_codon <- paste0(alt_codon_chars, collapse = "")
        aaalt <- translate_codon(alt_codon)
        # Only keep true AA substitutions (exclude synonymous)
        if (!identical(aaalt, aaref)) {
          k <- k + 1L
  ...
  if (k == 0L) {
    res <- data.frame(aapos = integer(), aaref = character(), aaalt = character(),
                      stringsAsFactors = FALSE)
    return(res)
  }
  df <- do.call(rbind, out_list[seq_len(k)])
  
  if (return_unique) {
    # Collapse to unique AA substitutions per position
    df <- unique(df[c("aapos", "aaref", "aaalt")])
    rownames(df) <- NULL
  }
  df
}
